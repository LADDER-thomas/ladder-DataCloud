...
アクセスキーは datacloud/aws_credentials.json を優先して読む（無いときは環境変数・~/.aws を利用）。
...
# 入力CSVのあるフォルダ（datacloud の親）。カレントが datacloud のときと同等。
_DEFAULT_WORK_DIR = (
    _SCRIPT_DIR.parent if _SCRIPT_DIR.name == "datacloud" else _SCRIPT_DIR
)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="input_file の相対パスの基準ディレクトリ（既定: datacloud の親＝共有データフォルダ）",
    )
